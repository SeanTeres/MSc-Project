...
for experiment_name, experiment in config['experiments'].items():
    ...
    # Create dataloaders using stored indices
    train_loader_d1, val_loader_d1, test_loader_d1 = helpers.create_dataloaders(d1, split_indices['d1'], batch_size, oversam=oversampling)
    train_loader_d2, val_loader_d2, test_loader_d2 = helpers.create_dataloaders(d2, split_indices['d2'], batch_size, oversam=oversampling)
    ...
    print("Label distribution for training set (D1):", helpers.calc_label_dist(d1, train_loader_d1.dataset, target))
    print("Label distribution for validation set (D1):", helpers.calc_label_dist(d1, val_loader_d1.dataset, target))
    print("Label distribution for test set (D1):", helpers.calc_label_dist(d1, test_loader_d1.dataset, target))
    
    print("Label distribution for training set (D2):", helpers.calc_label_dist(d2, train_loader_d2.dataset, target))
    print("Label distribution for validation set (D2):", helpers.calc_label_dist(d2, val_loader_d2.dataset, target))
    print("Label distribution for test set (D2):", helpers.calc_label_dist(d2, test_loader_d2.dataset, target))
    ...
    if train_dataset == "MBOD 1":
        ...
        model = train_utils.train_model(train_loader_d1, val_loader_d1, model, n_epochs, lr, device, loss_type=experiment['loss'])
    else:
        print("Training on Dataset 2\n")
        model = train_utils.train_model(train_loader_d2, val_loader_d2, model, n_epochs, lr, device, loss_type=experiment['loss'])

    test_labels_d1, test_preds_d1 = train_utils.test_model(test_loader_d1, model, device, "MBOD 1", loss_type=experiment['loss'])
    test_labels_d2, test_preds_d2 = train_utils.test_model(test_loader_d2, model, device, "MBOD 2", loss_type=experiment['loss'])
